...
| Logistic regression | Bernoulli/logit family, independence, linearity in the log-odds, no severe multicollinearity, usable class balance | Binary outcome at 27% churn, one row per customer ID; binned empirical log-odds of tenure checked directly against a straight line; VIF run on the full predictor set (`TotalCharges` ≈ 10.8, `MonthlyCharges` ≈ 866); test ROC-AUC = 0.832; recall = 0.516 at the default 0.5 threshold | Log-odds linearity fails for tenure: it drops sharply in year one, then flattens, which the GAM also shows and a single slope averages over and hides. Severe multicollinearity among `tenure`, `MonthlyCharges`, and `TotalCharges` makes their individual coefficients unstable — dropping `TotalCharges` flips the story for `MonthlyCharges`. The 0.5 cutoff misses about half of actual churners. |
...
| Logistic regression | Accuracy 0.787, precision 0.621, recall 0.516, F1 0.564, ROC-AUC 0.832 | Odds ratios with a hold-everything-else-fixed reading: tenure 0.24 (strongest predictor once features are standardized), two-year contract 0.57, fiber 1.92, online security 0.86 | One slope per feature, so it cannot show that risk is front-loaded in tenure; `TotalCharges`/`MonthlyCharges` swing to large, opposite-signed coefficients from multicollinearity (VIF up to 866) and should not be read individually; odds are not probabilities |
